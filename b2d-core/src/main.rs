...
async fn main() {
    ...
    let mut screen_rect = Rect{x: 0., y: 0., w: screen_width(), h: screen_height()};
    let mut cam2d = Camera2D::from_display_rect(screen_rect);
    set_camera(&cam2d);
    ...
    loop {
        ...
        set_default_camera();
        draw_text(&format!("x {:.2}, y{:.2}", scene.main_camera.transform.location.x, scene.main_camera.transform.location.y), 20.0, 20.0, 30.0, DARKGRAY);
